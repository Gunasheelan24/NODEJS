...
//Sending Token Via Email Address Only and Find The User There Or not
...
<html lang="en">
  ...
  <body>
    ...
    <p class="center">
      <a href="https://parisairwaysversionone.onrender.com/ParisAirways/v1/verfyToken/${token}/${email}" class="ancor"> Reset Password </a>
    </p>
    <p class="para">
...
//Just A Redirest Pase To Change Actual Password And Validate Token Time
//Weather It's Expired or Not
  ...
  try {
    ...
    if (userEmail.isexpire <= time) {
      ...
      if (token === userEmail.resetToken) {
        ...
        return res.redirect(
          200,
          `https://parisairwaysversion.onrender.com/ParisAirways/v1/resetPassword/${email}`
        );
      } else {
        res.status(401).json({
          status: "error",
...
//FinalStep Getting The Password and remove Reset Token
      ...
      if (verify === false) {
        ...
        res.status(200).json({
          ...
          result: {
            ...
            username: user.name,
            id: user._id,
            profile: `https://parisairwaysversionone.onrender.com/${user.profilePicture}`,
          },
...
exports.checkOut = async (req, res, next) => {
  ...
  const myDomain = "https://parisairwaysversion.onrender.com";
  const payment = {
